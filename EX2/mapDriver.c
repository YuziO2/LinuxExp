...
/*vm.area nopage */
static unsigned int map_fault(struct vm_fault *vmf);
...
static int __init mapdrv_init(void) /* 装载函数 */
{
	...
	for (virt_addr = (unsigned long)vmalloc_area; virt_addr < (unsigned long)vmalloc_area + MAPLEN; virt_addr += PAGE_SIZE)
	{
		...
		printk((char *)virt_addr, "test % d", i++);
	}
...
static unsigned int map_fault(struct vm_fault *vmf)
{
	...
	get_page(page);
	vmf->page = page;
	printk("%s: map ox %lx to ox %lx, size: ox %lx,page: %ld \n", __func__, virt_start, vmf->address, PAGE_SIZE, vmf->pgoff);
	return 0;
}
